...
import com.pixxel.Main.GenerateScriptFunctions;
...
//	private float prevSizeY = 0;
	...
	public interface TriggerProcessEvent {
		void start(String eventType);
		void executed(int exitCode);
	}
	private ArrayList<TriggerProcessEvent> events = new ArrayList<>();
	...
	public void setScript(String command) {
		...
		process.setApplicationListener(exitCode -> {
			for(TriggerProcessEvent events : events) events.executed(exitCode);
			executing = false;
	...
	public void leaveTrigger(WorldObject leaver) {
		...
		process.setVariable("leaver", leaver, false, true);
		process.removeVariable("event");
		process.setVariable("event", "onleave", false, true);
	...
	public void removeTrigger() {
		...
		process.removeVariable("event");
		process.setVariable("event", "onremove", false, true);
		execute();
	}
	...
	public void invokeTrigger() {
		...
		process.removeVariable("event");
		process.setVariable("event", "invoke", false, true);
		execute();
	}
	...
	public void customEvent(String eventName) {
		...
		process.removeVariable("event");
		process.setVariable("event", eventName, false, true);
		execute();
	}
	...
	public void execute() {
		...
		try {
			...
			for(TriggerProcessEvent event : events) event.start(process.getVariable("event", null).toString());
			process.execute(values.scr, true);
			Logger.logInfo("", "Executing script...");
		} catch(Exception e) {
	...
	public void addProcessEvents(TriggerProcessEvent event) {
