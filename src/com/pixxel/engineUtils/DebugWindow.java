package com.pixxel.engineUtils;
...
import com.pixxel.ai.Connection;
...
public class DebugWindow implements Disposable {
	...
	public DebugWindow(Game game, ArrayList<com.pixxel.objects.RootObject> preloaded, Skin skin) {
	...
	private void init() {
		...
		editTrigger.addListener(new ChangeListener() {
			public void changed(ChangeEvent event, Actor actor) {
				...
				com.pixxel.objects.WorldObject worldObjectedit = selectedObjects.get(0).object;
		...
		address.addListener(new ChangeListener() {
			public void changed(ChangeEvent event, Actor actor) {
				for(SelectedObject s : selectedObjects) {
					...
					if (s.object.getAddress().equals(com.pixxel.objects.WorldObject.NO_ADDR) || s.object.getAddress().equals(com.pixxel.objects.WorldObject.TILE_ADDR)) {
						game.stage.getObjectWithAddress().remove(s.object);
					} else {
						boolean found = false;
						for(com.pixxel.objects.WorldObject obj : game.stage.getObjectWithAddress()) {
	...
	public void drawVertices(RootObjectValues object, float x, float y, ShapeRenderer renderer) {
		...
		if(object.fixtures != null) {
			for (com.pixxel.objects.RootValues.Fixture f : object.fixtures) {
				if (f.isCircle) {
	...
	private com.pixxel.ai.Node connectMount = null;
	...
	public void update() {
		...
		if(neighborPerfect.isChecked() && selectedObjects.size() == 1 && selected != null) {
			com.pixxel.objects.WorldObject obj = selectedObjects.get(0).object;
		...
		if(Gdx.input.isKeyPressed(Keys.CONTROL_LEFT) && Gdx.input.isKeyPressed(Keys.D)) {
			if(game.stage.selected != null && !addGraphNode) game.stage.removeObject(game.stage.selected);
				...
				for(com.pixxel.ai.Node n : game.stage.getGraph().getNodes()) {
		...
		if((Gdx.input.isKeyPressed(Keys.CONTROL_LEFT)) && Gdx.input.isKeyJustPressed(Keys.S)) {
			if(game.stage.selected != null) {
				for (com.pixxel.objects.RootObject r : list) {
		...
		if(Gdx.input.isButtonPressed(Buttons.RIGHT) && !rubberbandActive && !addGraphNode) {
			...
			if(rubberbandPointer.dst(game.stage.getMousePos()) > 10*game.stage.getScale()) {
				...
				for (com.pixxel.objects.WorldObject w : game.stage.getRenderedObjects()) {
		...
		if((!Gdx.input.isTouched() || !addGraphNode) && connectMount != null) {
			for (com.pixxel.ai.Node n : game.stage.getGraph().getNodes()) {
				if (game.stage.getMousePos().dst(n.data.x * game.stage.getScale(), n.data.y* game.stage.getScale()) < game.stage.objectHitbox * game.stage.getScale() && !connectMount.equals(n)) {
					com.pixxel.objects.WorldValues.ConnectionData connectionData = new com.pixxel.objects.WorldValues.ConnectionData();
				...
				if(game.stage.selected == null && activeTab == 0) {
					if(!onWindow) {
						...
						if(!addGraphNode) {
							for (AddObject a : addObjectData) {
								if (a.addObjectRoot != null) {
									if (!tile.isChecked()) {
										...
										com.pixxel.objects.WorldObject obj = game.stage.addObject(a.addObjectValues, a.addObjectRoot);
							...
							com.pixxel.objects.WorldValues.NodeData n = new com.pixxel.objects.WorldValues.NodeData();
		...
		if(addGraphNode && game.stage != null) {
		...
		if(showHitboxes.isChecked()) {
			...
			for (com.pixxel.objects.WorldObject obj : game.stage.getRenderedObjects()) {
		...
		if(showHitboxes.isChecked()) {
			...
			for (com.pixxel.objects.Chunk c : game.stage.loadedChunks) {
	...
	private void selectWorldObject(com.pixxel.objects.WorldObject object) {
	...
	public void setSelected(com.pixxel.objects.RootObject root) {
		...
		for(Node n : main.getNodes()) {
			...
			if(a instanceof com.pixxel.ui.ObjectHolder) {
				if(((com.pixxel.ui.ObjectHolder) a).getObject() instanceof com.pixxel.objects.RootObject) {
					if(((com.pixxel.objects.RootObject) ((com.pixxel.ui.ObjectHolder) a).getObject()).getID().equals(root.getID())) {
	...
	public void addObject(com.pixxel.objects.RootObject object) {
		Table prev = new Table(skin);
		com.pixxel.ui.ObjectHolder<com.pixxel.objects.RootObject> holder = new com.pixxel.ui.ObjectHolder<>(object);
	...
	public class SelectedObject {
		...
		public com.pixxel.objects.WorldObject object;

		SelectedObject(Vector2 offset, com.pixxel.objects.WorldObject object) {
			this.offset = offset;
			this.object = object;
		}
	}
	...
	class AddObject {
		...
		com.pixxel.objects.RootObject addObjectRoot = null;
		Vector2 offset;

		AddObject(WorldObjectValues addObjectValues, com.pixxel.objects.RootObject addObjectRoot, Vector2 offset) {
	...
	private ArrayList<com.pixxel.objects.RootObject> preloaded;
	private CheckBox neighborPerfect;
	private com.pixxel.objects.RootObject selected;
	private ArrayList<com.pixxel.objects.RootObject> list;
				...
				new Thread(new Runnable() {
				    ...
				    public void run() {
						...
						if (res == JFileChooser.APPROVE_OPTION) {
							try {
								com.pixxel.files.JsonHandler jh = new JsonHandler();
								RootObjectValues values = jh.readJSON(chooser.getSelectedFile().getAbsolutePath(), RootObjectValues.class);
								com.pixxel.objects.RootObject r = new com.pixxel.objects.RootObject(values);
...
//			}});
	...
	private com.pixxel.objects.Behavior selectedBehavior;
	...
	private void initEditBehaviorWindow() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				try {
					if(setField.getType().toString().equals("float")) {
						if(com.pixxel.utils.Tools.Convert.testForFloat(valueInput.getText())) com.pixxel.objects.Behavior.setVariable(selectedBehavior, setField.getName(), Float.valueOf(valueInput.getText()));
						else return;
					} else if(setField.getType().toString().equals("int")) {
						if(com.pixxel.utils.Tools.Convert.testForInteger(valueInput.getText())) com.pixxel.objects.Behavior.setVariable(selectedBehavior, setField.getName(), Integer.valueOf(valueInput.getText()));
						else return;
					} else if(setField.getType().toString().equals("boolean")) {
						if(Tools.Convert.testForBoolean(valueInput.getText())) com.pixxel.objects.Behavior.setVariable(selectedBehavior, setField.getName(), Boolean.valueOf(valueInput.getText()));
						else return;
					} else com.pixxel.objects.Behavior.setVariable(selectedBehavior, setField.getName(), valueInput.getText());
		...
		for (Class<?> c : com.pixxel.objects.Behavior.BEHAVIOR_CLASSES) {
			a.add(c.getName());
		}
		...
		classes.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {
				id.setText(com.pixxel.objects.Behavior.BEHAVIOR_CLASSES.get(classes.getSelectedIndex()).getSimpleName());
			}
		});
	...
	public void setBehaviorTabs(com.pixxel.objects.WorldObject object) {
		...
		for(com.pixxel.objects.Behavior b : object.getBehavior()) {
			...
			try {
				Field[] validFields = com.pixxel.objects.Behavior.getFields(b);
	...
	private Table initTab12(float initWidth, float initHeight) {
		...
		main = new Tree(skin);
		list = new ArrayList<>();
		for(com.pixxel.objects.RootObject r : preloaded) {
			r.loadTexture();
			list.add(r);
		}

		for(com.pixxel.objects.RootObject obj : preloaded)	addObject(obj);
			...
			public void changed(ChangeEvent event, Actor actor) {
				...
				if(!main.getSelection().isEmpty()) {
					...
					if(main.getSelection().first().getActor() instanceof Label) {
						...
						if (a instanceof com.pixxel.ui.ObjectHolder) {
							if (((com.pixxel.ui.ObjectHolder) a).getObject() instanceof com.pixxel.objects.RootObject)
								selected = (com.pixxel.objects.RootObject) ((ObjectHolder) a).getObject();
