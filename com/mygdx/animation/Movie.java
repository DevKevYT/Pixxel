...
public class Movie {
	...
	/**Set directories to load all sprites in subdirectory*/
	//public Movie(int frameRate, final FileHandle path) {
	//	if(frameRate == 0) throw new IllegalArgumentException("The framerate can't be zero");
	//	if(!path.isDirectory()) frames.add(Movie.load(path.path()));
	//	else {
	//		for(FileHandle sub : path.list()) {
	//			if(!sub.isDirectory()) frames.add(Movie.load(sub.path()));
	//		}
	//	}
	//	this.frameRate = frameRate;
	//}
