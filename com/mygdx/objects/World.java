...
import com.mygdx.objects.WorldValues.WorldData;
...
public final class World implements Disposable {
	...
	protected final ArrayList<RootObject> library = new ArrayList<>();
	...
	public static final RootObject MISSING;
	...
	static {
		...
		MISSINGVAL.texturePath = "textures//missing.png";
		MISSING = new RootObject(MISSINGVAL);
	}

	public World(Game game, SpriteBatch batch) {
		this.batch = batch;
	...
	public void generate(FileHandle file, ArrayList<RootObject> objectlibrary) {
	...
	public void generate(Game game, FileHandle file, ArrayList<RootObject> objectlibrary) {
		if(file == null) throw new IllegalArgumentException("File not found!");
		if(!file.exists()) throw new IllegalArgumentException("File not found! " + file.path());
		library.clear();
		library.addAll(objectlibrary);
	...
	private void generate(WorldData data) {
		Gdx.app.postRunnable(new Runnable() {
			public void run() {
				...
				width = worldData.sizeX * tilesizeNORM * WorldValues.GroundValues.SQUARE;
				height = worldData.sizeY * tilesizeNORM * WorldValues.GroundValues.SQUARE;

				for(RootObject obj : library) obj.checkSize(World.this);  //Check library
				...
				if(worldData.sizeX > 0 && worldData.sizeY > 0) {
					...
					for (int i = 0; i < worldData.sizeX; i++) {
						for (int j = 0; j < worldData.sizeY; j++) {
							...
							chunks[i][j] = new Chunk(World.this, new WorldValues.ChunkData(), i, j);
							if (worldData.viewport.rd <= 0) chunks[i][j].load();
						}
					}
				}
				...
				if(Gdx.app.getType() == Application.ApplicationType.Android) worldData.viewport.rd = WorldValues.ViewportData.androidRD;
	...
	public RootObject getByID(String id) {
		for(RootObject obj : library) {
	...
	public void draw(float delta) {
		...
		for(Chunk chunk : loadedChunks) {
			for(int x = 0; x < WorldValues.GroundValues.SQUARE; x++) {  //A single chunk
				for(int y = 0; y < WorldValues.GroundValues.SQUARE; y++) {
	...
	public void changeTile(RootObject root, int tileX, int tileY) {
		int chunkTileX = tileX / WorldValues.GroundValues.SQUARE;
		int chunkTileY = tileY / WorldValues.GroundValues.SQUARE;
		if(chunkTileX < 0 || chunkTileX >= worldData.sizeX || chunkTileY < 0 || chunkTileY >= worldData.sizeY) return;
		changeTile(root, chunks[chunkTileX][chunkTileY], tileX % WorldValues.GroundValues.SQUARE, tileY % WorldValues.GroundValues.SQUARE);
	}
	
	public void changeTile(RootObject object, Chunk chunk, int tileX, int tileY) {
		if(tileX >= 0 && tileY >= 0 && tileX < WorldValues.GroundValues.SQUARE && tileY < WorldValues.GroundValues.SQUARE) {
			boolean wasLoaded = chunk.loaded();
			chunk.load();  //Theres no way around...

			RootObject newtileRoot = object.copy();  //Root configuration as tile
	 ...
	 * I recomment to use the {@link World#addObject(WorldObjectValues, RootObject)} function, where the
	 * root object is already set.*/
	public WorldObject addObject(WorldObjectValues values) {
		RootObject root = getByID(values.id);
	...
	public ArrayList<Tile> getTilesInRadius(float x, float y, int sizeX, int sizeY, boolean wake, ArrayList<Tile> list) {
		...
		int shiftX = (currentTile.getX() - sizeX + sizeX * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE;  //Dude this was pain. Sat a long time on this...
		int chunkStepX = (sizeX + ((-sizeX + currentTile.getX() + sizeX * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE)) / WorldValues.GroundValues.SQUARE;

		int shiftY = (currentTile.getY() - sizeY + sizeY * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE;
		int chunkStepY = (sizeY + ((-sizeY + currentTile.getY() + sizeY * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE)) / WorldValues.GroundValues.SQUARE;

		for(int i = /*-sizeX*/0, xIndex=0; i<=sizeX; i++, xIndex++) {
			int indexX = (i + currentTile.getX() + sizeX* WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE;
			int chunkIndexX = ((xIndex + shiftX) / WorldValues.GroundValues.SQUARE) - chunkStepX + currentChunk.x;

			for(int j = /*-sizeY*/0, yIndex=0; j <= sizeY; j++, yIndex++) {

				int indexY = (j + currentTile.getY() + sizeY * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE;
				int chunkIndexY = ((yIndex + shiftY) / WorldValues.GroundValues.SQUARE) - chunkStepY + currentChunk.y;
	...
	public ArrayList<WorldObject> getObjectsInRadius(float x, float y, int radius, boolean wake, ArrayList<WorldObject> list) {
		...
		int shiftX = (currentTile.getX() - radius + radius* WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE;  //Dude this was pain. Sat a long time on this...
		int chunkStepX = (radius + ((-radius + currentTile.getX() + radius * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE)) / WorldValues.GroundValues.SQUARE;

		int shiftY = (currentTile.getY() - radius + radius * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE;
		int chunkStepY = (radius + ((-radius + currentTile.getY() + radius * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE)) / WorldValues.GroundValues.SQUARE;

		for(int i = -radius, xIndex=0; i<=radius; i++, xIndex++) {
			int indexX = (i + currentTile.getX() + radius* WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE;
			int chunkIndexX = ((xIndex + shiftX) / WorldValues.GroundValues.SQUARE) - chunkStepX + currentChunk.x;

			for(int j = -radius, yIndex=0; j <= radius; j++, yIndex++) {

				int indexY = (j + currentTile.getY() + radius * WorldValues.GroundValues.SQUARE) % WorldValues.GroundValues.SQUARE;
				int chunkIndexY = ((yIndex + shiftY) / WorldValues.GroundValues.SQUARE) - chunkStepY + currentChunk.y;
	...
	public WorldData getFullData() {
		...
		data.chunks = new WorldValues.ChunkData[data.sizeX][data.sizeY];
		for(int i = 0; i < data.sizeX; i++) {
			for(int j = 0; j < data.sizeY; j++) {
				...
				data.chunks[i][j] = new WorldValues.ChunkData();
	...
	public Chunk estimateChunk(float x, float y) {
		...
		int chunkX = (int) (x / tilesizeNORM / WorldValues.GroundValues.SQUARE);
		int chunkY = (int) (y / tilesizeNORM / WorldValues.GroundValues.SQUARE);
	...
	public Tile estimateTile(float x, float y, boolean wake) {
		...
		int tileX = (int) (x / tilesizeNORM) % WorldValues.GroundValues.SQUARE;
		int tileY = (int) (y / tilesizeNORM) % WorldValues.GroundValues.SQUARE;

		if(tileX < 0) tileX = 0;
		else if(tileX > WorldValues.GroundValues.SQUARE) tileX = WorldValues.GroundValues.SQUARE-1;

		if(tileY < 0) tileY = 0;
		else if(tileY >= WorldValues.GroundValues.SQUARE) tileY = WorldValues.GroundValues.SQUARE-1;
