...
public class ItemInfo extends Actor {
    ...
    public ItemInfo(Game game, Stage stage, Skin skin) {
       ...
       // itemName.setFontScale(0.5f);
        ...
        windowTable.row();
        destoryProgress = new ProgressBar(0, 1, 0.01f, false, skin, "destroyitem");
        destoryProgress.setVisible(false);
        windowTable.add(destoryProgress).colspan(2).expandX();
        ...
        dismantle.addAction(new Action() {
            ...
            public boolean act(float delta) {
                if(dismantle.getClickListener().isOver() && Gdx.input.isButtonPressed(Input.Buttons.LEFT)) {
                    if(cell != null) {
                        destoryProgress.setVisible(true);
                        destoryProgress.setValue(destoryProgress.getValue() + (delta / cell.itemRoot.data.destroyTime));
                        if (destoryProgress.getValue() == destoryProgress.getMaxValue()) {
                            cell.dismantle();
                            destoryProgress.setValue(0);
                            destoryProgress.setVisible(false);
                        }
                    }
                } else {
    ...
    public void setDisplay(PlayerInventory.ItemCell cell) {
        ...
        windowTable.getCell(destoryProgress).expand();
